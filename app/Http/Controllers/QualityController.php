...
class QualityController extends Controller
{
            ...
            return DataTables::eloquent($model)
                ...
                ->addColumn('applicantPhone', function ($applicant) {
                    ...
                    if (!empty(trim($applicant->applicant_phone_secondary))) {
                        $parts[] = DialLink::render($applicant->applicant_phone_secondary, 'Secondary Phone');
                    }
            ...
            return DataTables::eloquent($model)
                ...
                ->addColumn('applicantPhone', function ($applicant) {
                    ...
                    if (!empty(trim((string) $applicant->applicant_phone))) {
                        $parts[] = \App\Support\DialLink::render($applicant->applicant_phone, 'Primary Phone');
                    }
                    if (!empty(trim((string) $applicant->applicant_phone_secondary))) {
                        $parts[] = \App\Support\DialLink::render($applicant->applicant_phone_secondary, 'Secondary Phone');
                    }
                    if (!empty(trim((string) $applicant->applicant_landline))) {
                        $parts[] = \App\Support\DialLink::render($applicant->applicant_landline, 'Landline');
                    }
        ...
        return DB::table('cv_notes as cn')
            ->joinSub(
                DB::table('cv_notes')
                    ...
                    ->groupBy('applicant_id', 'sale_id'),
                'cn_max',
                fn($j) => $j->on('cn.applicant_id', '=', 'cn_max.applicant_id')
                    ->on('cn.sale_id', '=', 'cn_max.sale_id')
                    ->on('cn.id', '=', 'cn_max.max_id')
            )
            ->select([
                'cn.applicant_id',
                'cn.sale_id',
    ...
    public function clearRejectSale(Request $request)
    {
        ...
        try {
            ...
            return DB::transaction(function () use ($id, $status, $notes, $user) {
    ...
    public function updateApplicantStatusByQuality(Request $request)
    {
        ...
        try {
            ...
            return DB::transaction(function () use ($applicant_id, $sale_id, $notes, $status, $user, $details) {
            if ($status == 'rejected') {
    ...
    public function getQualityNotesHistory(Request $request)
    {
        try {
            ...
            if ($qualityNotes->isEmpty()) {
                return response()->json(['error' => 'Quality note not found'], 404);
            }

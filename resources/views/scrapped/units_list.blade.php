@extends('layouts.vertical', ['title' => 'Scrapped Units List', 'subTitle' => 'Scrap'])
@section('style')
    <style>
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                            @canany(['unit-export-all'])
                                                <a class="dropdown-item export-btn"
                                                    href="{{ route('unitsExport', ['type' => 'scrapped-all']) }}">Export All
                                                    Data</a>
                                            @endcanany
                                            @canany(['unit-export-emails'])
                                                <a class="dropdown-item export-btn"
                                                    href="{{ route('unitsExport', ['type' => 'scrapped-emails']) }}">Export
                                                    Emails</a>
                                            @endcanany
                                        </div>
                                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            let columns = [{
                ...
                {
                    data: 'contact_email',
                    name: 'unit_contacts.contact_email'
                },
                {
                    data: 'contact_phone',
                    name: 'unit_contacts.contact_phone'
                },
                {
                    data: 'contact_landline',
                    name: 'unit_contacts.contact_landline'
                },
                {
                    data: 'unit_notes',
            ...
            var table = $('#units_table').DataTable({
                ...
                ajax: {
                    url: @json(route('getScrappedUnits')), // Fetch data from the backend
                    type: 'GET',
                    data: function(d) {
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            Swal.fire(
                                'Deleted!',
                                response.message || 'Unit(s) has been deleted.',
                                'success'
                            );
        ...
        $('#bulk-approve-btn').on('click', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "This will automatically approve the unit along with its associated office.",
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('scrapped.unit.approve') }}",
                        type: 'POST',
                        data: {
                            id: ids,
                            _token: $('meta[name="csrf-token"]').attr('content')
